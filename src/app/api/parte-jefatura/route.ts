...
export async function POST(req: Request) {
  try {
    ...
    if (contentType.includes('application/x-www-form-urlencoded') || contentType.includes('multipart/form-data')) {
      ...
      body = payload ? (JSON.parse(String(payload)) as Body) : ({} as Body);
      const download = form.get('download');
      if (download) {
        (body as Body & { download?: boolean }).download = true;
      }
    } else {
    ...
    console.error('parte-jefatura PDF error:', error);
    const details = error instanceof Error ? error.message : String(error);
    return NextResponse.json({ error: 'Error generando PDF.', details }, { status: 500 });
  }
}
