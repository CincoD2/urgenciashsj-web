...
export default function UreaBunPage() {
  ...
  const unitLabelUrea = useMemo(
    () => UNIDADES.find((u) => u.id === ureaUnit)?.label ?? 'mg/dL',
    [ureaUnit]
  );
  const unitLabelBun = useMemo(
    () => UNIDADES.find((u) => u.id === bunUnit)?.label ?? 'mg/dL',
    [bunUnit]
  );
...
*BUN: Blood Urea Nitrogen`;
  ...
  return (
    <main className="escala-wrapper space-y-6" style={{ padding: 24 }}>
      ...
      <div className="resultado amarillo">
        <div className="interpretacion">
          Fórmulas: BUN* = Urea / 2.1428 | Urea = 2.1428 × BUN* | Urea (mmol/L) = 0.357 × BUN*
          (mg/dL)
        </div>
      </div>
